...
![image-20210625185119140](img/image-20210625185119140.png)

2，将userArgumentResolver类注入到List<HandlerMethodArgumentResolver>中

![image-20210625190144765](img/image-20210625190144765.png)
...
![image-20210528210436097](img/image-20210528210436097.png)

前端：
![image-20210626152744663](img/image-20210626152744663.png)

![image-20210626152813780](img/image-20210626152813780.png)

后端：

![image-20210626152929694](img/image-20210626152929694.png)

![image-20210626153721555](img/image-20210626153721555.png)
...
![image-20210626155209813](img/image-20210626155209813.png)
...
![image-20210626155403784](img/image-20210626155403784.png)

**使用拦截器实现通用限流**：

**1.自定义@AccessLimit注解**

![image-20210626161118436](img/image-20210626161118436.png)

**2.创建拦截器**

![image-20210626161307410](img/image-20210626161307410.png)

![image-20210626161700367](img/image-20210626161700367.png)
...
![image-20210806162426796](img/image-20210806162426796.png)
...
```shell
//进入mysql容器
docker exec -it 887ea1e3f271 /bin/bash
//进入mysql
...
![image-20210904125831405](img/image-20210904125831405.png)
...
```shell
docker run 
-p 6379:6379    #宿主端口与容器端口映射 
-v /usr/data/redis/data:/data #挂载，将容器中的redis持久化数据挂载到宿主机，避免容器重启导致数据丢失
--restart=always #无论什么情况出错，总是重启
--name redis #容器名称
-d redis:latest redis-server #使用指定的镜像运行容器 
...
![image-20210904131358317](img/image-20210904131358317.png)
...
![image-20210904133323207](img/image-20210904133323207.png)
...
（2）模拟用户登录，获取token，并将token存到txt文件中

（3）使用JMeter工具读取txt文件中token，设置线程数，模拟用户登录

## 2.具体实现：

（1）设置线程数以及循环次数

![image-20210904205222015](img/image-20210904205222015.png)
...
![image-20210904205322485](img/image-20210904205322485.png)
...
| 线程数（用户数） | 循环数 | 库存数 | 错误率 | Throughput |
| ---------------- | ------ | ------ | ------ | ---------- |
| 5000             | 1      | 50     | 0.00%  | 429.9      |
| 6000             | 1      | 50     | 0.00%  | 439.6      |
| 7000             | 1      | 50     | 0.00%  | 465.3      |
| 8000             | 1      | 50     | 0.00%  | 426.8      |
| 9000             | 1      | 50     | 0.00%  | 439.0      |
| 10000            | 1      | 50     | 0.00%  | 405.3      |

平均值：434.3

添加Redis以及RabbitMQ优化后：

| 编号 | 线程数（用户数） | 循环数 | 库存数 | 错误率 | Throughput |
| ---- | ---------------- | ------ | ------ | ------ | ---------- |
| 1    | 5000             | 1      | 50     | 0.00%  | 2259.4     |
| 2    | 6000             | 1      | 50     | 0.00%  | 2247.2     |
| 3    | 7000             | 1      | 50     | 0.00%  | 2215.9     |
| 4    | 8000             | 1      | 50     | 0.00%  | 2124.8     |
| 5    | 9000             | 1      | 50     | 0.00%  | 2097.9     |
| 6    | 10000            | 1      | 50     | 0.00%  | 2405.0     |
...
```mysql
//创建唯一索引
CREATE UNIQUE index gid on order_info(goods_id,user_id)
//删除唯一索引
alter table order_info drop index gid
//展示唯一索引
...
![image-20210903134453823](img/image-20210903134453823.png)
